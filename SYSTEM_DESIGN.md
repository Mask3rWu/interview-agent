...
interviewAgent/                         # 项目根目录
  apps/                                 # 应用代码目录
    web/                                # Next.js 前端应用
      ...
      components/                       # 前端通用组件：表格、选择器、对话框、聊天消息
      lib/                              # 前端工具：API client、SSE client、格式化函数
      types/                            # 前端 TypeScript 类型，与后端响应结构保持一致

    api/                                # FastAPI 后端应用
      app/                              	# 后端 Python 包根目录
        main.py                         		# FastAPI app 创建、路由注册、启动入口
        api/                            		# HTTP API 层，只处理请求响应和参数校验
        routes/                       			# 按业务拆分的 FastAPI router
...
api           # HTTP 层：依赖 services / schemas / core，负责请求响应
...
- `schemas` 不能 import `services`、`agents`、`api`。
- `db` 不能 import `agents`、`api`。
- `services` 不能 import `api`。
- `agents` 可以调用 `services`，但 `services` 不反向调用 `agents`。
- `api/routes` 只做请求校验、调用 service、返回响应，不直接写数据库或调用模型。
...
```text
apps/api/app/logs/model_calls.log
```
